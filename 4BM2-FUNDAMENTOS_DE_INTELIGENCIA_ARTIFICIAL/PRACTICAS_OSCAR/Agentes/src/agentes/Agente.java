...
public class Agente extends Thread {
    ...
    public Agente(String nombre, ImageIcon icon, int[][] matrix, JLabel tablero[][], ImageIcon mother, ImageIcon migaja, ImageIcon pacman, HashMap< Integer, ImageIcon> cluster) {
        this.nombre = nombre;
        this.icon = icon;
        this.matrix = matrix;
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (i == yBase && j == xBase) { // Comprueba si ha llegado a la nave
            icon = pacman; // Cambia la imagen de vuelta a la normal
            enNave = true;
            }

            if ((entregas + compañero.entregas) == objetivo && obtenidas == 0) {
    ...
    public synchronized void actualizarPosicion() {
        ...
        if (irBase) {
            ...
            //icon = new ImageIcon("imagenes/pacmanbola.png");
    ...
    public synchronized boolean sensorBola() {
    try {
        // Verifica si la casilla actual es un "sample"
        if (matrix[i][j] == 12) {
            // Cambia la imagen del agente a "pacmanbola"
            icon = new ImageIcon("imagenes/pacmanbola.png");
            icon = new ImageIcon(icon.getImage().getScaledInstance(30, 30, java.awt.Image.SCALE_SMOOTH));
            return true;
        }
