...
void apilar(Pila *p, int* error, struct NodoP dato){
    struct NodoP* nodoUsuario=(struct NodoP*) malloc(sizeof(struct NodoP));
    if(nodoUsuario==NULL){
...
void desapilardesapilar(Pila *p, int* error,struct NodoP *datoExtraido){
    if(!isEmpty(*p,error)){
        struct NodoP* aux=p->cima;
