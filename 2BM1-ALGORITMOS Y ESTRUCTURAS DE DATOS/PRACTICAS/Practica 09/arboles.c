...
void cantidadHojas(Arbol raiz , int* count){
    if(raiz -> izq == NULL && raiz -> der == NULL)
            (*count)++;
    if(raiz){
        cantidadHojas(raiz -> izq , *count);
        cantidadHojas(raiz -> der , *count);
    }
}
    ...
    if(raiz){
        if(elementoBuscado.valor==(*raiz)->info.valor){
            *nodoEncontrado=(*raiz);
            ...
            alturaArbol(&((*raiz)->der),elementoBuscado,nodoEncontrado , (*count)++);
        } else{
            alturaArbol(&((*raiz)->izq),elementoBuscado,nodoEncontrado , (*count)++);
        }
    }
}